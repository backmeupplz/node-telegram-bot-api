...
var TelegramBot = function (_EventEmitter) {
  ...
  _createClass(TelegramBot, [{
    ...
    value: function processUpdate(update) {
      ...
      var successfulPayment = update.successful_payment;
      ...
      if (message) {
        ...
        debug('Process Update successful_payment %j', successfulPayment);
        this.emit('successful_payment', successfulPayment);
      }
    }
    ...
    key: 'sendInvoice',
    value: function sendInvoice(chatId) {
    ...
    value: function answerPreCheckoutQuery(queryId) {
      var form = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

      form.pre_checkout_query_id = queryId;
